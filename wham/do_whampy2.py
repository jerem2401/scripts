...
def do_wham(z, histos, pos, kappa, RT = 2.49434, tol = 1e-6, bCycl = False,
    ...
    # if (bCycl):
    #     if (not (cycl_max > cycl_min)):
    #         print 'Error, cycl_max must be larger than cycl_min'
    #         sys.exit(0)

    #     icycl_min = int(round((cycl_min-z[0])/dz))
    #     icycl_max = int(round((cycl_max-z[0])/dz))

    #     if (icycl_min < 0):
    #         icycl_min = 0
    #     if (icycl_max > nz_input-1):
    #         icycl_max = nz_input-1

        # Map histograms over periodic boundary
        ...
        #     for j in range(nz_input):
        #         if histos[i][j] > 0:
        #             j_min = min(j_min, j)
        #             j_max = max(j_max, j)
        # print 'Determined boundaries:'
        # print '\tLower bound = %10g  (index %d)' % (z[j_min], j_min)
        # print '\tUpper bound = %10g  (index %d)' % (z[j_max], j_max)
    
    # Bounds should only be given with bCycl
    if (bCycl):
