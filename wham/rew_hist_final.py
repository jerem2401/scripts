...
def main():
    ...
    hist2, bin_edges = np.histogram(df['nCV'], bins=[i for i in np.around(np.arange(-1.0,1.0,0.01), decimals=3)], weights=[np.exp(i/2.49434) for i in df['guide_restraint.bias']])
