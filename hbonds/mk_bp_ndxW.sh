...
dir="${dirumb}/hbond_${grp}"
if [[ $grp == Protein ]]; then
	traj='umb2.xtc'
	tpr='subset.tpr'
elif [[ $grp == Water ]]; then
	traj='solvant2.xtc'
	#first egrep just in case dirumb!=E_([0-9]|\.){5,6} (if launched from another dir for ex)
	ksi=$(echo $dirumb | egrep -o 'E_([0-9]|\.){5,6}' | egrep -o '([0-9]|\.){5,6}')
	tpr="conf_${ksi}.tpr"
else
	echo "you should give Protein or Water as group"
	exit
fi
...
if (($mknd == 1)); then
	...
	while read line; do
		...
		if [[ ! -z $fa3 ]]; then
			mkdir "${dir}/DNAt_${f}"
			sel=$(echo "group \"$grp\" and within \
			0.4 of (group \"DNAt\" and resid $f and ($fa1 or $fa2 or $fa3));")
			echo $sel > "${dir}/DNAt_${f}/selection.dat"
			gmx select -sf "${dir}/DNAt_${f}/selection.dat" -f ${dirumb}/${traj} \
			-s ${dirumb}/${tpr} -n $index -on ${dir}/DNAt_${f}/hbindex.ndx

			mkdir "${dir}/DNAnt_${s}"
			sel=$(echo "group \"$grp\" and within \
			0.4 of (group \"DNAnt\" and resid $s and ($sa1 or $sa2 or $sa3));")
			echo $sel > "${dir}/DNAnt_${s}/selection.dat"
			gmx select -sf "${dir}/DNAnt_${s}/selection.dat" -f ${dirumb}/${traj} \
			-s ${dirumb}/${tpr} -n $index -on ${dir}/DNAnt_${s}/hbindex.ndx
		else
			mkdir "${dir}/DNAt_${f}"
			sel=$(echo "group \"$grp\" and within \
	...
	for i in ${dir}/DNA*; do
		...
		if (($atominrange == 0)); then
			...
			-s ${dirumb}/${tpr} -n $index -on ${i}/hbindexdna.ndx

			cat ${i}/hbindexm.ndx ${i}/hbindexdna.ndx > ${i}/final.ndx
			echo "0 1" | gmx hbond -nthreads 1 -f ${dirumb}/${traj} \
			-s ${dirumb}/${tpr} -n ${i}/final.ndx -num "${i}/hbond.xvg"
			wait
